...
CREATE UNIQUE INDEX uq_gcs_active
    ON group_class_students (group_class_id, student_user_id)
    WHERE status = 'ACTIVE';
...
CREATE TABLE group_class_recurrence (
                                        ...
                                        CONSTRAINT check_group_class_recurrence_status CHECK (status IN ('SCHEDULED', 'COMPLETED', 'CANCELLED', 'REMOVED', 'COMPLETED_WITHOUT_PAYMENTS'))
);
...
CREATE TABLE recurrence_class_student (
                                          id SERIAL PRIMARY KEY,
                                          status VARCHAR(20) NOT NULL DEFAULT 'SCHEDULED' CHECK (status IN ('SCHEDULED', 'ATTENDED', 'ABSENT', 'CANCELLED', 'TUTOR_CANCELLED')),
                                          ...
                                          payment_method_selected VARCHAR(20) CHECK (payment_method_selected IN ('CASH', 'FREE_LESSON')),
